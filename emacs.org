...
*** TODO Remove when emacs 25 is released because this is included
    Sometimes I uncompress .zip files from within emacs. This lets me do it in dired with ~Z~
...
** Passwords
   ...
   #+END_SRC
** Highlights
   This package provides a visual cue whenever we change a block of text by highlighting it for a bit:
...
* Helm
  Helm is a incremental completion and selection narrowing framework for emacs. It's super powerful and super customizable, much like the rest of emacs. Let's load it up! I also set up my keybindings here.
...
** Helm and syntax checking
   We use flycheck for syntax checking. Helm can be integrated into this.
   ...
   #+BEGIN_SRC emacs-lisp
     (use-package org-ref
       ...
       (add-hook 'org-mode-hook
                 (lambda () (local-set-key (kbd "C-c r") #'org-ref-helm-insert-cite-link)))
       (eval-after-load "latex"
         '(define-key LaTeX-mode-map (kbd "C-c r") #'org-ref-helm-insert-cite-link))
       (eval-after-load 'markdown-mode
         '(define-key markdown-mode-map (kbd "C-c r") #'org-ref-helm-insert-cite-link))
